...
//Esta es la primera funcionalidad (Carga de datos)
...
//Esta es la segunda funcionalidad (Ingreso a la plataforma)
    ...
    do{
        ...
        case 1:{
            ...
            switch (control2) {
            case 1:{  //opcion para agregar canciones a la lista de favoritos
                ...
                if(ActualUser->get_MiListaRP()->AgregarCancion(BuscarCancionPorID(Canciones,CCanciones,codeCancionAUX,x),x)){
            ...
            case 2:{//opcion para eliminar canciones de la lista de favoritos
                cout<<"Su lista de favoritos es: "<<'\n';
                ActualUser->get_MiListaRP()->MostrarLista();
                cout<<"Ingrese el codigo de la cancion que desea eliminar de su lista de favoritos: ";
                cin>>codeCancionAUX;
                if(ActualUser->get_MiListaRP()->BuscarCancionPorCodigo(codeCancionAUX)==nullptr){
                    cout<<"La cancion con codigo "<<codeCancionAUX<<" no se encuentra en su lista de favoritas"<<'\n';
                    break;
                }
                ActualUser->get_MiListaRP()->EliminarCancion(ActualUser->get_MiListaRP()->BuscarCancionPorCodigo(codeCancionAUX),x);
